...
$definitions = @(
    ...
    [ordered]@{ sourceName = "MetaTFT Japanese lookup"; sourceUrl = [string]$meta.sources.metaTftJapaneseLookup; terms = "Public endpoint; availability and terms must be monitored"; count = @($meta.compositions | Where-Object { $_ -is [pscustomobject] }).Count; fallback = $metaPath },
