...
配信サイトは250MiBを上限とし、70%（175MiB）、85%（212.5MiB）、95%（237.5MiB）で警告します。新版公開時にactive履歴を最新版込み5版へ自動整理します。最新available・stable正常版・直前availableを保護し、残りを新しい順に保持するため、新セット収集中でも正常版を失いません。過去PATCH/NEW_SETを永久に配信しません。未使用blobも除去し、`archive-map.json` は直近50件に限定します。上限そのものは自動拡張せず、整理後も警告域へ達した場合は原因を調べます。Git履歴の容量は別であり、この整理ではGit履歴の書き換えや削除を行いません。
