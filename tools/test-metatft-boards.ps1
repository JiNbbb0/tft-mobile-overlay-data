...
$details = [pscustomobject]@{
    early_options = [pscustomobject]@{
        ...
        '6' = @(
            [pscustomobject]@{
                unit_list='A&B&DA_Elderwood18_Lifeblossom&C&D&E&F'
                count=500
                avg=4.0
                positions=@('cell_1','cell_2','cell_3','cell_4','cell_5','cell_6','cell_7')
            }
        )
    }
...
$boards = @(Convert-MetaTftLevelBoards -Details $details -AllowedUnitIds $allowedUnitIds)
...
Assert-Equal ($level6.unitIds -join '&') 'A&B&C&D&E&F' 'Non-champion source entities must be filtered from canonical boards'
Assert-Equal @($level6.unitIds | Where-Object { $_ -eq 'DA_Elderwood18_Lifeblossom' }).Count 0 'Summoned helper entities must not survive board conversion'
Assert-Equal $level6.positionsAvailable $true 'Explicit level-specific positions should be retained after helper filtering'
Assert-Equal (($level6.positions | ForEach-Object { $_.position }) -join ',') '0,1,3,4,5,6' 'Explicit cells must preserve original source indexes after helper filtering without vertical inversion'
